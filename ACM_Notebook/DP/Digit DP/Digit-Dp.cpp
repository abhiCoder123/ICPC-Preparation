// Question :
// How many numbers x are there in the range a to b, where digit "d" occurs exactly k times in x ?
...
// DP[p][c][f] = Number of valid numbers <=b from this stated
...
int call(int pos,int cnt,int f){
	...
	if(f==0){
		...
		// So if we placed any digit > num[pos] in the current position, then the number will be greater than b
		LMT=num[pos];
	}else{
